...
def load_data(df_name: str) -> pd.DataFrame:
    ...
    elif df_name == 'Qualitative Bankruptcy':
        return load_Bankrupcy()
...
def load_cars() -> pd.DataFrame:
    ...
    url = 'https://raw.githubusercontent.com/kardam00/Lazy_FCA/main/car.csv'
...
def load_seismic() -> pd.DataFrame:
    ...
    url = 'https://raw.githubusercontent.com/kardam00/Lazy_FCA/main/seismic-bumps.csv'
    df = pd.read_csv(url)
...
def load_ThoraricSurgery() -> pd.DataFrame:
    ...
    url = 'https://raw.githubusercontent.com/kardam00/Lazy_FCA/main/ThoraricSurgery.csv'
    df = pd.read_csv(url)
...
def load_Bankrupcy() -> pd.DataFrame:

    #url = 'https://raw.githubusercontent.com/kardam00/Lazy_FCA/main/Qualitative_Bankruptcy.csv'
    df = pd.read_csv('https://raw.githubusercontent.com/kardam00/Lazy_FCA/main/Qualitative_Bankruptcy.csv')
    
    df.columns = ['Industrial Risk','Management Risk','Financial Flexibility','Credibility',
                  'Competitiveness','Operating Risk', 'class']
    df['class'] = [x == 'B' for x in df['class']]
    return df

def load_Breast_Cancer() -> pd.DataFrame:

    url = 'https://raw.githubusercontent.com/kardam00/Lazy_FCA/main/Breast_Cancer.csv'
    df = pd.read_csv(url)
